...
import OnlineTicTacToeView from './OnlineTicTacToeView';

export default function TicTacToePage() {
  const navigate = useNavigate();
  const [gameMode, setGameMode] = useState(null); // null | 'multiplayer' | 'ai' | 'online'
  const {
    board,
    isXNext,
    gameOver,
    ...
    return (
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(124,111,255,0.12),transparent_24%),linear-gradient(180deg,#111937_0%,#0b0f1e_52%,#090d1a_100%)] px-4 py-6 text-copy-50 sm:px-6">
        <div className="mx-auto w-full max-w-2xl">
          <OnlineTicTacToeView onBack={() => setGameMode(null)} />
        </div>
      </div>
    );
  }
